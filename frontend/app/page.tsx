...
import AuthButtons from "@/components/AuthDropdown";
...
const faqs = [
  {
    ...
    answer:
      "No. MindEase is a supportive self-help companion. For severe distress or emergencies, professional or crisis care is essential.",
  },
  {
    question: "Are my conversations actually private?",
    answer:
      "Yes! We use end-to-end encryption at rest for all conversations. Your sensitive data is locked securely and cannot be read by anyone else.",
  },
  {
    question: "Can I use this if I am not ready to talk openly?",
    answer:
      "Absolutely. You can begin anonymously, use the structured journaling tools, or try shorter prompts until you feel comfortable.",
  },
];
...
export default function HomePage() {
  return (
    ...
    >
      {/* Navigation */}
      <nav className="relative z-50 mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-6 md:px-10">
      ...
      <section className="relative z-10 mx-auto flex w-full max-w-6xl flex-col items-center justify-center px-6 pb-20 pt-12 text-center md:px-10 md:pt-16 lg:pt-20">
        {/* Scoped background orbs - only on homepage hero */}
        <div className="home-orb home-orb-left absolute pointer-events-none" />
        <div className="home-orb home-orb-right absolute pointer-events-none" />
        ...
        <p className="reveal-up reveal-delay-2 mt-6 max-w-2xl text-lg text-neutral-300 md:text-xl leading-relaxed">
          MindEase is an AI-powered companion helping students navigate stress,
          burnout, and emotional overload through confidential conversations and
        ...
        <div className="reveal-up reveal-delay-3 mt-10 flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          <Link
            href="/chat"
            className="group relative inline-flex items-center justify-center gap-2 overflow-hidden rounded-2xl bg-white px-8 py-4 font-bold text-neutral-950 transition-all hover:scale-105 hover:shadow-[0_0_40px_rgba(255,255,255,0.3)]"
          >
          ...
          <Link
            href="/voice"
            className="inline-flex items-center justify-center gap-2 rounded-2xl border border-white/20 bg-white/5 px-8 py-4 font-semibold text-white backdrop-blur-md transition-all hover:bg-white/10 hover:border-white/30"
          >
        ...
        <div className="reveal-up reveal-delay-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {quickStats.map((stat, i) => {
            ...
            return (
              <div
                key={i}
                className="glass-panel glass-panel-hover flex flex-col items-center justify-center rounded-2xl p-6 text-center"
              >
      ...
      <section className="relative z-10 mx-auto w-full max-w-6xl px-6 pb-32 md:px-10">
        <div className="text-center mb-16">
          <h2 className="font-[var(--font-heading)] text-3xl font-extrabold text-white md:text-5xl">
            How MindEase Works
          </h2>
          <p className="mt-4 text-neutral-400 max-w-2xl mx-auto">
            A simple, intuitive flow designed to provide support exactly when
            you need it.
          </p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3 relative">
          ...
          {processSteps.map((step, i) => {
            ...
            return (
              <div
                key={i}
                className="reveal-up glass-panel glass-panel-hover rounded-3xl p-8 relative"
                style={{ animationDelay: `${i * 0.15}s` }}
              >
                <div className="absolute -top-6 left-8 rounded-2xl bg-gradient-to-br from-teal-500 to-blue-600 p-4 shadow-lg">
                  <Icon className="h-6 w-6 text-white" />
                </div>
                <h3 className="mt-6 font-[var(--font-heading)] text-xl font-bold text-white">
                  {step.title}
                </h3>
      ...
      <section className="relative z-10 mx-auto w-full max-w-6xl px-6 pb-32 md:px-10">
        <div className="mb-12 text-center md:text-left">
          ...
          <p className="mt-4 text-neutral-400 max-w-2xl">
            Everything you need to build mental resilience, in one secure
            platform.
          </p>
        </div>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {featuredItems.map((item, i) => (
            <div
              key={item.id}
              className="reveal-up glass-panel glass-panel-hover group rounded-3xl p-8"
              style={{ animationDelay: `${i * 0.1}s` }}
            >
              <h3 className="font-[var(--font-heading)] text-xl font-bold text-white group-hover:text-teal-300 transition-colors">
                {item.title}
              </h3>
      ...
      <section className="relative z-10 mx-auto w-full max-w-3xl px-6 pb-32 md:px-10 text-center">
        <h2 className="font-[var(--font-heading)] text-3xl font-extrabold text-white md:text-4xl mb-10">
        ...
        <div className="space-y-4 text-left">
          {faqs.map((faq, i) => (
            <details
              key={i}
              className="reveal-up group glass-panel rounded-2xl overflow-hidden [&_summary::-webkit-details-marker]:hidden"
              style={{ animationDelay: `${i * 0.1}s` }}
            >
      ...
      <footer className="relative z-10 border-t border-white/10 bg-black/40 backdrop-blur-md">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-8 md:px-10">
          ...
          <p className="flex-1 text-center text-sm text-neutral-500">
            © {new Date().getFullYear()} MindEase. Designed for student
            wellness.
          </p>
