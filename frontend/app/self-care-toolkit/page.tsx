...
export default function CopingPage() {
  ...
  const fetchStrategies = async () => {
    try {
      const data = await apiFetch<StrategiesResponse>("/coping");
  ...
  const generateStrategies = async () => {
    ...
    setGenerating(true);
    try {
      const data = await apiFetch<StrategiesResponse>("/coping/generate", {
        method: "POST",
      ...
      const message =
        e instanceof Error ? e.message : "Failed to generate strategies.";
      alert(message);
    } finally {
      setGenerating(false);
    }
  ...
  const toggleSave = async (id: string) => {
    try {
      const data = await apiFetch<StrategyResponse>(`/coping/${id}/save`, {
        method: "POST",
      });
  ...
  const markComplete = async (id: string) => {
    try {
      const data = await apiFetch<StrategyResponse>(`/coping/${id}/complete`, {
        method: "POST",
      });
      setStrategies(strategies.map((s) => (s.id === id ? data.strategy : s)));
  ...
  const rateStrategy = async (id: string, rating: number) => {
    try {
      const data = await apiFetch<StrategyResponse>(`/coping/${id}/rate`, {
        method: "POST",
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <p className="text-neutral-400 mt-1">
              AI-powered strategies tailored to your situation
            </p>
          </div>
          ...
          <div className="rounded-xl border border-neutral-800 bg-neutral-900/50 p-5 mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                className={`text-xs font-medium px-3 py-1 rounded-full bg-gradient-to-r ${levelColors[progress.currentLevel] || levelColors.beginner} text-white capitalize`}
              >
            ...
            <div className="mt-3 h-2 bg-neutral-800 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(100, (progress.totalCompleted / 15) * 100)}%`,
                }}
              />
            </div>
            <p className="text-xs text-neutral-500 mt-1.5">
              {progress.currentLevel === "advanced"
                ...
                : `${progress.currentLevel === "beginner" ? 5 - progress.totalCompleted : 15 - progress.totalCompleted} more to level up`}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-neutral-800 bg-neutral-900/50 p-6 mb-6">
            <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-teal-400" /> Generate New
              Strategies
            </h2>
            <textarea
              value={situation}
          ...
          {loading ? (
            ...
            <div className="text-center py-12 text-neutral-500">
              <p className="text-xl mb-2">
                {tab === "saved" ? "No saved strategies" : "No strategies yet"}
              </p>
              <p className="text-sm">
                Describe your situation above to get personalized strategies
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {filtered.map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className="rounded-xl border border-neutral-800 bg-neutral-900/50 overflow-hidden hover:border-neutral-700 transition group"
                  >
                    {/* Card Header */}
                    <div className="p-5">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-start gap-3 flex-1">
                          <span className="text-2xl mt-0.5">
                            {practiceTypeIcons[s.practiceType] || "✨"}
                          </span>
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex flex-wrap gap-2 mt-1.5">
                              ...
                              <span
                                className={`text-xs border px-2 py-0.5 rounded-full capitalize ${difficultyColors[s.difficulty] || difficultyColors.beginner}`}
                              >
                                {s.difficulty}
                              </span>
                        ...
                        <div className="flex gap-1">
                          <button
                            ...
                            className="p-1.5 hover:bg-neutral-800 rounded-lg transition cursor-pointer"
                          >
                            {s.isSaved ? (
                              <BookmarkCheck
                                size={18}
                          ...
                          <button
                            ...
                            className="p-1.5 hover:bg-red-600/20 rounded-lg transition opacity-0 group-hover:opacity-100 cursor-pointer"
                          >
                        ...
                        <ol className="space-y-2.5">
                          {s.steps.map((step, i) => (
                            <li key={i} className="flex items-start gap-3">
                              <span className="flex-shrink-0 w-6 h-6 rounded-full bg-teal-600/20 text-teal-400 flex items-center justify-center text-xs font-bold">
                                {i + 1}
                              </span>
                              <span className="text-sm text-neutral-300 pt-0.5">
                                {step}
                              </span>
                            </li>
                          ))}
                        ...
                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-neutral-800">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {[1, 2, 3, 4, 5].map((star) => (
                              <button
                                key={star}
                                onClick={() => rateStrategy(s.id, star)}
                                className="cursor-pointer p-0.5 hover:scale-125 transition"
                              >
                                <Star
                                  className={`w-4 h-4 ${s.rating && star <= s.rating ? "text-yellow-400 fill-yellow-400" : "text-neutral-600"}`}
                                />
                              </button>
                            ))}
                          </div>
