...
import { Heart, Flame, HandHeart, Send, Plus, X, MessageCircle, Sparkles, Users, ChevronDown, ChevronUp, Trash2 } from "lucide-react";

interface Story {
  id: string; title: string; content: string; category: string; tags: string[];
  isAnonymous: boolean; displayName: string; supportCount: number;
  commentCount: number; userSupport: string | null; createdAt: string;
  isApproved?: boolean;
}

interface Comment {
  id: string; content: string; displayName: string; createdAt: string; isOwn: boolean;
}
...
const reactionIcons: Record<string, { icon: any; label: string; color: string }> = {
  heart: { icon: Heart, label: "Love", color: "text-pink-400 hover:text-pink-300" },
  strength: { icon: Flame, label: "Strong", color: "text-orange-400 hover:text-orange-300" },
  hug: { icon: HandHeart, label: "Support", color: "text-purple-400 hover:text-purple-300" },
};
...
export default function CommunityPage() {
  ...
  const [newStory, setNewStory] = useState({ title: "", content: "", category: "general", isAnonymous: true });
  ...
  const fetchStories = async (cat = "all") => {
    ...
    try {
      ...
      const data = await apiFetch(`/stories${params}`);
  ...
  const fetchMatched = async () => {
    try {
      const data = await apiFetch("/stories/matched");
  ...
  const fetchMyStories = async () => {
    try {
      const data = await apiFetch("/stories/mine");
  ...
  const submitStory = async () => {
    if (!newStory.title.trim() || !newStory.content.trim() || submitting) return;
    setSubmitting(true);
    try {
      ...
      if (data.moderation?.isApproved) {
        ...
        setSuccessMsg("Your story has been submitted for review. We'll publish it once verified.");
      }
      setNewStory({ title: "", content: "", category: "general", isAnonymous: true });
  ...
  const handleSupport = async (storyId: string, type: string) => {
    try {
      const data = await apiFetch(`/stories/${storyId}/support`, {
        method: "POST", body: JSON.stringify({ type }),
      });
      const updateStory = (s: Story) =>
        s.id === storyId ? { ...s, supportCount: data.supported ? s.supportCount + 1 : s.supportCount - 1, userSupport: data.supported ? type : null } : s;
  ...
  const toggleComments = async (storyId: string) => {
    if (expandedComments === storyId) { setExpandedComments(null); return; }
  ...
  const submitComment = async (storyId: string) => {
    ...
    try {
      const data = await apiFetch(`/stories/${storyId}/comments`, {
        method: "POST", body: JSON.stringify({ content: commentInput }),
      });
      if (data.isApproved) {
        setComments((prev) => ({ ...prev, [storyId]: [...(prev[storyId] || []), data.comment] }));
      }
      setCommentInput("");
    } catch (e) { console.error(e); }
  ...
  const renderStoryCard = (story: Story, showDelete = false) => (
    <div key={story.id} className="rounded-xl border border-neutral-800 bg-neutral-900/50 p-5 hover:border-neutral-700 transition">
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          <h3 className="font-semibold text-white text-lg">{story.title}</h3>
          <div className="flex items-center gap-2 mt-1.5">
            <span className="text-xs text-neutral-500">{story.displayName}</span>
            <span className="text-xs text-neutral-700">•</span>
            <span className="text-xs text-neutral-500">{new Date(story.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
            <span className="text-xs bg-neutral-800 border border-neutral-700 px-2 py-0.5 rounded-full capitalize">{story.category.replace("_", " ")}</span>
            {story.isApproved === false && <span className="text-xs bg-yellow-500/20 text-yellow-400 border border-yellow-500/30 px-2 py-0.5 rounded-full">Pending Review</span>}
          </div>
        </div>
        {showDelete && (
          <button onClick={() => deleteStory(story.id)} className="p-1.5 hover:bg-red-600/20 rounded-lg transition cursor-pointer">
            <Trash2 size={16} className="text-red-400" />
          </button>
        )}
      </div>

      <p className="text-sm text-neutral-300 leading-relaxed whitespace-pre-wrap mb-4">{story.content}</p>

      {story.tags && story.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-3">
          {story.tags.map((tag, i) => <span key={i} className="text-xs bg-blue-500/10 text-blue-300 border border-blue-500/20 px-2 py-0.5 rounded-full">{tag}</span>)}
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-t border-neutral-800 pt-3">
        <div className="flex items-center gap-1">
          {Object.entries(reactionIcons).map(([type, { icon: Icon, label, color }]) => (
            <button key={type} onClick={() => handleSupport(story.id, type)}
              ...
              <Icon className={`w-4 h-4 ${story.userSupport === type ? "fill-current" : ""}`} />
              <span className="text-xs">{label}</span>
            </button>
          ))}
          <span className="text-xs text-neutral-500 ml-2">{story.supportCount}</span>
        </div>
        ...
        <button onClick={() => toggleComments(story.id)}
          ...
          {expandedComments === story.id ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
        </button>
      </div>
        ...
        <div className="mt-3 border-t border-neutral-800 pt-3">
                ...
                <div key={c.id} className="flex items-start gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-neutral-300">{c.displayName}</span>
                      <span className="text-xs text-neutral-600">{new Date(c.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                    </div>
                    <p className="text-sm text-neutral-400 mt-0.5">{c.content}</p>
                  </div>
                </div>
              ))
            )}
          ...
          <div className="flex gap-2">
            <input type="text" value={commentInput} onChange={(e) => setCommentInput(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") submitComment(story.id); }}
            ...
            <button onClick={() => submitComment(story.id)} disabled={commentLoading || !commentInput.trim()}
  ...
  return (
    <div className="flex h-screen text-white">
      ...
      <main className="flex-1 overflow-y-auto px-4 py-8">
        {successMsg && (
          <div className="fixed top-4 right-4 bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg z-50">{successMsg}</div>
        )}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-neutral-400 mt-1">Share your story, support others, grow together</p>
            </div>
          ...
          {/* New Story Modal */}
          {showNewStory && (
            <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setShowNewStory(false)}>
              <div className="bg-neutral-900 border border-neutral-700 rounded-xl p-6 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold text-purple-400">Share Your Story</h2>
                  <button onClick={() => setShowNewStory(false)} className="p-1 hover:bg-neutral-700 rounded cursor-pointer"><X size={18} /></button>
                </div>
                <input type="text" value={newStory.title} onChange={(e) => setNewStory({ ...newStory, title: e.target.value })}
                ...
                <textarea value={newStory.content} onChange={(e) => setNewStory({ ...newStory, content: e.target.value })} rows={6}
                ...
                <div className="flex flex-wrap gap-3 mb-4">
                  <select value={newStory.category} onChange={(e) => setNewStory({ ...newStory, category: e.target.value })}
                    className="px-1 py-2 bg-black border border-neutral-700 rounded-lg text-sm text-white outline-none">
                    {categories.filter((c) => c.value !== "all").map((c) => <option key={c.value} value={c.value}>{c.label}</option>)}
                  </select>
                  <button onClick={() => setNewStory({ ...newStory, isAnonymous: !newStory.isAnonymous })}
                ...
                <p className="text-xs text-neutral-500 mb-4">🛡️ All stories are reviewed by our AI moderator to ensure a safe community</p>
                <button onClick={submitStory} disabled={submitting || !newStory.title.trim() || !newStory.content.trim()}
          ...
          {loading && tab === "browse" ? (
            ...
            matchedStories.length === 0 ? (
              <div className="text-center py-12 text-neutral-500">
                ...
                <p className="text-sm">Log some moods first so we can find stories that resonate with you</p>
              </div>
            ) : (
              <div>
                <p className="text-sm text-neutral-400 mb-4 flex items-center gap-2">
                  <Sparkles className="w-4 h-4 text-purple-400" /> Stories matched to your recent moods
                </p>
                <div className="space-y-4">{matchedStories.map((s) => renderStoryCard(s))}</div>
              </div>
            )
          ) : tab === "mine" ? (
            myStories.length === 0 ? (
              <div className="text-center py-12 text-neutral-500">
                <p className="text-lg mb-1">You haven't shared any stories</p>
                <p className="text-sm">Your experience could help someone else</p>
              </div>
            ) : (
              <div className="space-y-4">{myStories.map((s) => renderStoryCard(s, true))}</div>
            )
          ) : stories.length === 0 ? (
            <div className="text-center py-12 text-neutral-500">
            ...
            <div className="space-y-4">{stories.map((s) => renderStoryCard(s))}</div>
          )}
        </div>
      </main>
    </div>
