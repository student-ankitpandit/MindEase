...
export default function ChatPage() {
  ...
  // Voice call mode
  const [voiceCallMode] = useState(false);
  ...
  const initRecognition = useCallback(() => {
    ...
    return recognition;
  }, [clearSilenceTimer, speechLang, startSilenceTimer]);
  ...
  const speakTextIfEnabled = useCallback(
    (text: string) => {
  ...
  const loadConversation = async (externalId: string) => {
    ...
    try {
      const data = await apiFetch<ConversationDetailResponse>(
        `/chatWithMe/conversations/${externalId}`,
      );
    ...
    try {
      ...
      if (activeConvId) {
        data = await apiFetch<ChatResponse>(
          `/chatWithMe/conversations/${activeConvId}/messages`,
          {
            method: "POST",
      ...
      const aiResponse =
        data.res?.response || "Sorry, I couldn't generate a response.";
      const aiMsg: Message = {
        id: `ai-${Date.now()}`,
        content: aiResponse,
        role: "assistant",
  ...
  const deleteChat = async (execId: string) => {
    try {
      ...
      if (
        conversations.find((c) => c.id === execId)?.externalId === activeConvId
      ) {
        setActiveConvId(null);
        setMessages([]);
      }
  ...
  const generateSummary = async () => {
    ...
    try {
      const data = await apiFetch<SummaryResponse>(
        `/chatWithMe/conversations/${activeConvId}/summarize`,
        { method: "POST" },
      );
      setSummaryData({
  ...
  const voiceCallSend = useCallback(
    async (text: string) => {
    ...
    [activeConvId, initRecognition, speakText, startSilenceTimer],
  );
    ...
    <div className="flex flex-col h-screen">
        ...
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center">
          <div className="text-center">
            <p className="text-neutral-400 mb-4">
              Redirecting to Voice Conversation...
            </p>
            <div className="w-8 h-8 border-2 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto" />
          </div>
          <script
            dangerouslySetInnerHTML={{
              __html: `window.location.href = '/voice';`,
            }}
          />
        </div>
      )}
      ...
      <div className="h-14 border-b border-neutral-800 bg-neutral-950 flex items-center px-6 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs bg-purple-600/20 text-purple-400 border border-purple-500/30 px-2 py-0.5 rounded-full ml-1">
            Voice + Text
          </span>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            {sideLoading ? (
              ...
              conversations.map((conv) => (
                ...
                >
                  <MessageCircle
                    size={16}
                    className={`flex-shrink-0 transition-colors ${activeConvId === conv.externalId ? "text-blue-400" : "text-neutral-500 group-hover:text-neutral-300"}`}
                  />
        ...
        <div className="flex-1 flex flex-col relative">
          ...
          {showSummary && (
            ...
            >
              <div
                className="bg-neutral-900 border border-neutral-700 rounded-xl p-6 max-w-lg w-full max-h-[80vh] overflow-y-auto"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-lg font-bold text-purple-400 flex items-center gap-2">
                    <Sparkles className="w-5 h-5" /> Conversation Summary
                  </h2>
                  <button
                    onClick={() => setShowSummary(false)}
          ...
          <div className="flex-1 overflow-y-auto px-4 py-6">
            <div className="max-w-3xl mx-auto space-y-4">
              {messages.length === 0 && !loading && (
                <div className="flex flex-col items-center justify-center h-full text-center py-20">
                  ...
                  <p className="text-neutral-600 max-w-sm mb-4">
                    Ask me anything about managing stress, academics,
                    relationships, or personal growth.
                  </p>
                  <div className="flex flex-wrap gap-2 justify-center">
                    <span className="text-xs bg-neutral-800 border border-neutral-700 px-3 py-1.5 rounded-full text-neutral-400">
              ...
              {messages.map((msg) => (
                ...
                >
                  <div
                    className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${msg.role === "user" ? "bg-blue-600" : "bg-purple-600"}`}
                  >
                    {msg.role === "user" ? (
            ...
            {isListening && (
              <div className="max-w-3xl mx-auto mt-2 flex items-center justify-center gap-2">
                ...
                <span className="text-xs text-red-400">
                  Listening in{" "}
                  {languages.find((l) => l.code === speechLang)?.label}...
                </span>
              </div>
            )}
          </div>
