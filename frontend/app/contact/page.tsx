...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setStatus("loading");
    try {
      const res = await apiFetch("/feedback", { method: "POST", body: JSON.stringify(form) });
      setStatus("success");
    ...
    <div className="flex h-screen text-white">
      ...
      <div className="max-w-xl w-full">
        <div className="mb-8">
          ...
          <p className="text-neutral-400 mt-2">Have feedback or need support? We&apos;re here to listen.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input name="name" value={form.name} onChange={handleChange} required minLength={2} placeholder="Your name"
          ...
          <div>
            ...
            <input name="subject" value={form.subject} onChange={handleChange} required minLength={3} placeholder="What's this about?"
              className="w-full px-4 py-3 bg-neutral-900 border border-neutral-800 rounded-xl text-white placeholder-neutral-600 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all" />
          </div>
          <div>
            <label className="block text-sm text-neutral-400 mb-1.5">Message</label>
            <textarea name="message" value={form.message} onChange={handleChange} required minLength={10} rows={5} placeholder="Tell us what's on your mind..."
