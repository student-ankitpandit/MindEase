...
const moods = [
  { label: "Happy", emoji: "😄" }, { label: "Calm", emoji: "🙂" },
  { label: "Neutral", emoji: "😐" }, { label: "Sad", emoji: "😔" },
  { label: "Stressed", emoji: "😣" }, { label: "Angry", emoji: "😡" },
];
...
export default function MoodInputPage() {
  ...
  const [result, setResult] = useState<{ mood: string; advice: string } | null>(null);
  const [analytics, setAnalytics] = useState<Analytics | null>(null);
  const [analyticsLoading, setAnalyticsLoading] = useState(true);

  useEffect(() => {
    apiFetch("/getMoodSts/analytics")
  ...
  const handleSubmit = async () => {
    ...
    setResult(null);
    try {
      const data = await apiFetch("/moodSts/moodStatus", {
        method: "POST",
      ...
      setSelectedMood("");
      // Refresh analytics
      apiFetch("/getMoodSts/analytics").then((res) => setAnalytics(res.analytics)).catch(() => {});
    } catch (e: any) {
      console.error(e);
    } finally {
      setSubmitting(false);
    ...
    <div className="flex h-screen text-white">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="border border-neutral-800 rounded-2xl p-6 bg-neutral-900/50 mb-8">
          ...
          <p className="text-neutral-400 mb-6">Your mood helps us understand and support you better.</p>

          <label className="block text-neutral-300 mb-2">Describe your day:</label>
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={4} placeholder="Write anything... even one sentence is okay."
            ...
            {moods.map((m) => (
              <button key={m.label} onClick={() => setSelectedMood(m.label)}
                className={`px-4 py-2 rounded-xl border transition text-md cursor-pointer ${
                  selectedMood === m.label ? "bg-blue-600 border-blue-700 text-white" : "bg-black border-neutral-700 text-neutral-300 hover:bg-neutral-900"
                }`}>
                {m.emoji} {m.label}
              </button>
            ))}
        ...
        {result && (
          <div className="mb-8 border border-emerald-500/30 bg-emerald-500/5 rounded-xl p-5">
            <p className="text-emerald-400 font-semibold mb-2">AI detected mood: {moodEmojis[result.mood?.toLowerCase()] || "🫥"} {result.mood}</p>
        ...
        {!analyticsLoading && analytics && (
          <div className="space-y-6">
            <h2 className="text-2xl font-bold text-white">Your Mood Analytics</h2>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="rounded-xl border border-neutral-800 bg-neutral-900/50 p-5 text-center">
                ...
                <p className="text-2xl font-bold capitalize">{moodEmojis[analytics.mostFrequentMood?.toLowerCase()] || ""} {analytics.mostFrequentMood}</p>
              ...
              <div className="rounded-xl border border-neutral-800 bg-neutral-900/50 p-5">
                ...
                <div className="space-y-2">
                  {analytics.monthlyDistribution.map((item) => {
                    const maxCount = analytics.monthlyDistribution[0]?.count || 1;
                    const pct = Math.round((item.count / maxCount) * 100);
                    return (
                      <div key={item.mood} className="flex items-center gap-3">
                        <span className="text-lg w-6">{moodEmojis[item.mood?.toLowerCase()] || "🫥"}</span>
                        <span className="text-sm w-20 capitalize">{item.mood}</span>
